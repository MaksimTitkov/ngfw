...
### 2.1 NAT правила
- [x] UpdateNatRule — POST /api/v1/nat/rules/update, двойной клик по строке NAT → Edit модал
...
- [x] Сохранить правило как шаблон (кнопка «Template» в bulk-bar)
- [x] Библиотека шаблонов — страница /templates с карточками
- [x] Применить шаблон: выбрать устройство + папку → создать правило
- [x] Хранение в локальной БД (таблица rule_templates, не зависит от NGFW)
- [x] Удаление шаблонов (одиночное и bulk)
- [ ] Массовое применение на несколько device groups

### 7.5 Object Usage Map — карта использования объектов
Нет в оригинале.

- [x] Для любого объекта — показать все правила, где он используется (кнопка «Find in Rules» в /objects)
- [x] Поиск по объектам через /search?mode=object&q=<ext_id>
- [x] GET /api/v1/object/usage?ext_id=... — API для объектного поиска
...
- [x] **Экспорт политики в Excel** — GET /api/v1/export/rules/xlsx, цветовая кодировка action, auto-filter, имена объектов
- [x] **Экспорт структуры папок в YAML** — GET /api/v1/export/folders/yaml, полный snapshot папок + порядок правил
...
- [x] **Глобальный поиск** — Ctrl+K overlay на всех страницах, быстрый поиск по имени + IP + порт
- [x] **Поиск по IP** — находит объекты по IP, затем правила где они используются
- [x] **Поиск по порту** — находит сервисные объекты, затем правила
- [x] **Поиск по имени** — ILIKE по названию правила
- [x] **Страница /search** — расширенный поиск с режимами и фильтром по устройству
- [x] **POST /api/v1/search** — JSON API для quick search overlay
- [x] Результаты: имя, action, enabled, папка, устройство, причина совпадения + ссылка
